...
plt.scatter(np.interp(markers, df['Hкаб.'], df['filtered']), markers, color='red')
# print(np.interp(markers, df['Hкаб.'], df['filtered']))
# plt.plot(df2['MD'])
plt.plot()
plt.gca().invert_yaxis()
plt.hlines(markers, 0, np.interp(markers, df['Hкаб.'], df['filtered']), color='red')
plt.show()
